...
export default function HomePage() {
  ...
  const totalSpent = groups.reduce(
    (sum, group) =>
      sum +
      group.expenses.reduce((expenseSum, expense) => expenseSum + expense.amount, 0),
    0
  );
      ...
      <main className="mx-auto max-w-5xl px-4 py-6 sm:py-8">
        <div className="mb-8 animate-fade-in">
          ...
          <p className="max-w-2xl text-sm text-slate-500 sm:text-base">
            Split bills, track balances, and settle debts with a clean local-first
            workflow.
          </p>
              ...
              {[
                ...
                <div key={stat.label} className="glass-card flex items-center gap-3 p-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="truncate font-bold text-white">{stat.value}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {groups.length === 0 ? (
          <div className="glass-card animate-fade-in p-8 text-center sm:p-12">
            ...
            <p className="mx-auto mb-6 max-w-sm text-sm text-slate-500 sm:text-base">
              Create your first group for a trip, shared apartment, team outing, or any
              shared spending situation.
            </p>
            <button
              onClick={() => setShowCreate(true)}
              className="btn-primary mx-auto flex w-full items-center justify-center gap-2 sm:w-auto"
            >
              <Plus size={15} />
              Create First Group
            </button>
